"use client";

import React from "react";
import { Button } from "@/components/ui/button";
import { MoreVertical } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu";
import { useDrawerStore } from "@/store/drawer.store";
import DeleteModal from "@/components/molecules/DeleteModal";
import RiderDetails from "@/components/molecules/riders/RiderDetails";
import DisableRider from "./DisableRider";
import RiderEntity from "@/types/rider.type";

interface RiderActionsProps {
  rider: RiderEntity;
}

const RiderActions: React.FC<RiderActionsProps> = ({ rider }) => {
  const { openModal } = useDrawerStore();

  function onDelete(id: number) {
    console.log(`Deleting rider with id: ${id}`);
  }

  const handleApprove = () => {
    openModal({
      type: "dialog",
      title: "Disable Rider",
      content: DisableRider,
      props: { rider },
      placement: "center",
    });
  };

  const handleReject = () => {
    openModal({
      type: "dialog",
      title: "Disable Rider",
      content: DisableRider,
      props: { rider },
      placement: "center",
    });
  };

  const handleDelete = () => {
    openModal({
      type: "dialog",
      title: "Disable Rider",
      content: DeleteModal,
      props: {
        itemName: "Rider",
        onDelete: () => onDelete(rider.id),
      },
      placement: "center",
    });
  };

  const handleDisable = () => {
    openModal({
      type: "dialog",
      title: "Disable Rider",
      content: DisableRider,
      props: { rider },
      placement: "center",
    });
  };

  const handleViewDetails = () => {
    openModal({
      type: "drawer",
      title: "Rider Details",
      content: RiderDetails,
      props: { rider },
      placement: "right",
    });
  };

  const handleRejectModal = () => {
    openModal({
      title: "Reject Rider Request",
      content: DeleteModal,
      type: "dialog",
      props: {
        id: rider.id,
        title: "Reject Rider Request",
        message: "This action will permanently reject this rider.",
        confirmLabel: "Reject Rider",
        onConfirm: handleReject,
      },
      placement: "center",
    });
  };

  const handleDeleteModal = () => {
    openModal({
      title: "Delete Request",
      type: "dialog",

      content: DeleteModal,
      props: {
        id: rider.id,
        title: "Delete Request",
        message: "Are you sure you want to delete this rider request?",
        confirmLabel: "Delete Request",
        onConfirm: handleDelete,
      },
      placement: "center",
    });
  };

  const getMenuItems = () => {
    switch (rider.status) {
      case "PENDING":
        return (
          <>
            <DropdownMenuItem onSelect={handleViewDetails}>
              {/* <DrawerTrigger> */}
              View Details
              {/* </DrawerTrigger> */}
            </DropdownMenuItem>

            <DropdownMenuItem onSelect={handleApprove}>
              Approve Rider
            </DropdownMenuItem>

            <DropdownMenuItem
              onSelect={handleRejectModal}
              className="text-red-600"
            >
              Reject Rider
            </DropdownMenuItem>
          </>
        );

      case "APPROVED":
        return (
          <>
            <DropdownMenuItem onSelect={handleViewDetails}>
              View Details
            </DropdownMenuItem>

            <DropdownMenuItem onSelect={handleDisable}>
              Disable Rider
            </DropdownMenuItem>
          </>
        );

      case "DISABLED":
        return (
          <>
            <DropdownMenuItem onSelect={handleViewDetails}>
              View Details
            </DropdownMenuItem>

            <DropdownMenuItem
              onSelect={handleDeleteModal}
              className="text-red-600"
            >
              Delete Request
            </DropdownMenuItem>
          </>
        );

      default:
        return (
          <DropdownMenuItem onSelect={handleViewDetails}>
            View Details
          </DropdownMenuItem>
        );
    }
  };

  return (
    <DropdownMenu modal={false}>
      <DropdownMenuTrigger>
        <Button variant="ghost" size="icon">
          <MoreVertical className="h-4 w-4 text-gray-600" />
        </Button>
      </DropdownMenuTrigger>

      <DropdownMenuContent align="end">{getMenuItems()}</DropdownMenuContent>
    </DropdownMenu>
  );
};

export default RiderActions;

// "use client";

// import React, { useState } from "react";
// import { Button } from "@/components/ui/button";
// import { MoreVertical } from "lucide-react";
// import {
//   DropdownMenu,
//   DropdownMenuContent,
//   DropdownMenuItem,
//   DropdownMenuTrigger,
// } from "@/components/ui/dropdown-menu";
// import { useDrawerStore } from "@/store/drawer.store";
// import DeleteModal from "@/components/molecules/DeleteModal";
// import RiderDetails from "@/components/molecules/RiderDetails";
// import DisableRider from "./DisableRider";
// import RiderEntity from "@/types/rider.type";

// interface RiderActionsProps {
//   rider: RiderEntity;
// }

// const RiderActions: React.FC<RiderActionsProps> = ({ rider }) => {
//   const { openModal } = useDrawerStore();
//   const [open, setOpen] = useState(false);

//   /** --- HANDLERS --- **/
//   const handleOpen = (
//     type: "drawer" | "dialog",
//     title: string,
//     content: any,
//     props = {},
//     placement: "right" | "center" = "center"
//   ) => {
//     openModal({
//       type,
//       title,
//       content,
//       props,
//       placement,
//     });
//   };

//   const handleViewDetails = () =>
//     handleOpen("drawer", "Rider Details", RiderDetails, { rider }, "right");

//   const handleDisable = () =>
//     handleOpen("dialog", "Disable Rider", DisableRider, { rider });

//   const handleDelete = () =>
//     handleOpen("dialog", "Delete Request", DeleteModal, {
//       itemName: "Rider",
//       message: "Are you sure you want to delete this rider request?",
//       confirmLabel: "Delete",
//       onDelete: () => console.log(`Deleting rider with id: ${rider.id}`),
//     });

//   const handleReject = () =>
//     handleOpen("dialog", "Reject Rider Request", DeleteModal, {
//       itemName: "Rider",
//       message: "This action will permanently reject this rider.",
//       confirmLabel: "Reject Rider",
//       onDelete: () => console.log(`Rejecting rider with id: ${rider.id}`),
//     });

//   const handleApprove = () =>
//     console.log(`Approving rider with id: ${rider.id}`);

//   /** --- MENU ITEMS --- **/
//   const menuItems = {
//     PENDING: [
//       { label: "View Details", action: handleViewDetails },
//       { label: "Approve Rider", action: handleApprove },
//       { label: "Reject Rider", action: handleReject, danger: true },
//     ],
//     APPROVED: [
//       { label: "View Details", action: handleViewDetails },
//       { label: "Disable Rider", action: handleDisable },
//     ],
//     DISABLED: [
//       { label: "View Details", action: handleViewDetails },
//       { label: "Delete Request", action: handleDelete, danger: true },
//     ],
//   };

//   const items = menuItems[rider.status] ?? [
//     { label: "View Details", action: handleViewDetails },
//   ];

//   /** --- RENDER --- **/
//   return (
//     <DropdownMenu open={open} onOpenChange={setOpen}>
//       <div
//         onMouseEnter={() => setOpen(true)}
//         onMouseLeave={() => setOpen(false)}
//         className="relative inline-block"
//       >
//         <DropdownMenuTrigger asChild>
//           <Button
//             variant="ghost"
//             size="icon"
//             className="hover:bg-gray-100 focus:outline-none"
//           >
//             <MoreVertical className="h-4 w-4 text-gray-600" />
//           </Button>
//         </DropdownMenuTrigger>

//         <DropdownMenuContent align="end" className="z-50">
//           {items.map((item, idx) => (
//             <DropdownMenuItem
//               key={idx}
//               onSelect={(e) => {
//                 e.preventDefault();
//                 e.stopPropagation();
//                 item.action();
//               }}
//               className={item.danger ? "text-red-600" : ""}
//             >
//               {item.label}
//             </DropdownMenuItem>
//           ))}
//         </DropdownMenuContent>
//       </div>
//     </DropdownMenu>
//   );
// };

// export default RiderActions;
